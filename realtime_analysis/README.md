...
- **AI 内容品类结构**：自动分类为 7 类（学习/探索体验/具体应用/吃瓜/科普/前沿学术/教程·买课）+ 数据驱动补充 2 类（AI生成内容/观点讨论），含「供给-效率四象限定位」（蓝海/红海判断）与各品类代表笔记
...
品类分类在 `analyze_xhs.py` 的 `CATEGORY_RULES` 中定义（关键词 + 优先级，先命中者胜），可按自己的业务理解增删关键词或新增品类。
